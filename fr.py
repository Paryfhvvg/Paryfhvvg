...
CHANNELS_FILE = '1channels.json'
VPN_KEYS_FILE = '1vpn_keys.json'
USERS_FILE = '1users.json'
...
vpn_keys = load_data(VPN_KEYS_FILE, ["vpn_key1", "vpn_key2", "vpn_key3"])  # Список ключей, замените на свои
...
def process_add_channel_inline(message):
    try:
        ...
        # Экранируем link и name для отладочной строки
        escaped_link_debug = escape_markdown(link)
        escaped_name_debug = escape_markdown(name)
        ...
        bot.send_message(
            ...
            f"[Debug] Link: {escaped_link_debug}, Name: {escaped_name_debug}",
            reply_markup=keyboard,
            parse_mode="Markdown"
        )
        ...
        if e.error_code == 400 and "chat not found" in e.description.lower():
            bot.send_message(
                ...
                "2. Bot kanala administrator hökmünde gošuldy."
            )
        else:
            bot.send_message(message.chat.id, f"❌ Telegram API ýalňyšlygy: {e.description}")
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ Näbelli ýalňyšlyk: {str(e)}")
...
def confirm_add_channel(call):
    ...
    bot.send_message(call.from_user.id, f"✅ Kanal *{escape_markdown(name)}* üstünlikli gošuldy!", parse_mode="Markdown")
    bot.answer_callback_query(call.id)
    ...
    if channels:
        ...
        for channel in channels:
            ...
            try:
                ...
                bot.send_message(call.from_user.id, f"❌ Ýalňyšlyk kanal görkezilmedi: {e.description}")
        bot.send_message(
            call.from_user.id,
            "📜 *Häzirki kanallar:*",
            reply_markup=keyboard,
...
def confirm_add_vpn(call):
    ...
    if key not in vpn_keys:
        ...
        bot.send_message(call.from_user.id, f"✅ VPN açar *{escape_markdown(key)}* gošuldy!", parse_mode="Markdown")
    else:
