...
def init_db():
    try:
        with sqlite3.connect('vpn_botf.db') as conn:
            c = conn.cursor()
...
def main_menu(message):
    ...
    markup.add(types.InlineKeyboardButton("VPN alyň", callback_data="get_vpn"))
    if message.chat.id == ADMIN_ID:
        markup.add(types.InlineKeyboardButton("Admin Panel", callback_data="admin_panel"))

     bot.send_message(message.chat.id,
        "👋 *Paradox Sponsor Bot-a hoş geldiňiz!*\n\n"
        "VPN almak üçin:\n"
        "1. Aşakdaky kanallara abuna boluň.\n"
        "2. *VPN al* düwmesine basyň.\n\n"
        "Abunalygy barlarys we VPN size derrew bereris! 🚀",
